-- Additive support for the consolidated finance dashboard (Part 2/3 redesign):
-- (1) _finance_kpi_window gains shipping_collected + order_count (for the shipping-margin
--     and AOV tiles + the net-profit waterfall). Existing keys are unchanged, so the
--     reconciled gross/net/profit values stay identical (verified to the cent).
-- (2) finance_expense_breakdown returns expenses by business category for a window, so the
--     net-profit waterfall's Operating Expenses line can expand into postage / ads / fees.
...
create or replace function public.finance_expense_breakdown(p_org_id uuid, p_start date, p_end date)
...
language sql stable set search_path to 'public' as $function$
  ...
  where e.org_id = p_org_id and e.occurred_on >= p_start and e.occurred_on < p_end
  group by coalesce(e.category, 'Uncategorized')
  having coalesce(sum(e.amount), 0) <> 0
  order by sum(e.amount) desc;
$function$;

grant execute on function public.finance_expense_breakdown(uuid, date, date) to authenticated, anon, service_role;
