...
export default function CultivarProfit() {
  ...
  // Aggregate per-cultivar units & revenue from order_items.
  // Cost-of-goods is intentionally not derived (no per-cultivar cost field exists yet);
  // until that's tracked, we report units + revenue without margin.
  const rows = useMemo(() => {
    const map = new Map<string, { id: string; name: string; units: number; revenue: number; orders: number }>();
    for (const order of orders) {
      ...
      for (const item of order.items) {
        ...
        if (existing) {
          ...
          map.set(key, { id: key, name, units: item.qty, revenue, orders: 1 });
        }
        seen.add(key);
      }
    }
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto h-full flex flex-col">
      ...
      <p className="text-xs text-text-tertiary italic mt-4">
        Cost-of-goods isn't tracked per cultivar yet. Once we record propagation cost per batch, we can show margin here.
      </p>
    </div>
  );
}
